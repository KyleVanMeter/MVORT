...
class Render {
 public:
  ...
  /* --------------------------------------------------------------------------------------
     ...
     -------------------------------------------------------------------------------------- */
  void setCamera(Eigen::Vector3f lookfrom, Eigen::Vector3f lookat, Eigen::Vector3f vup, float vfov, float aspect,
...
Eigen::Vector3f Render::color(const Ray &r, Hitable *world, int depth) {
  ...
  if (world->hit(r, 0.001, MAXFLOAT, rec)) {
    ...
    if (depth < 50 && rec.mat_ptr->scatter(r, rec, attenuation, scattered)) {
      ...
      Eigen::Vector3f result(attenuation.x() * ret.x(), attenuation.y() * ret.y(), attenuation.z() * ret.z());
...
Hitable * Render::randomScene() {
  ...
  list[0] = new Sphere(Eigen::Vector3f(0, -1000, 0), 1000, new Lambertian(new ConstantTexture(Eigen::Vector3f(0.5, 0.5, 0.5))));
  int i = 1;
  for(int a = -11; a < 11; a++) {
    for(int b = -11; b < 11; b++) {
      ...
      if((center-Eigen::Vector3f(4,0.2,0)).norm() > 0.9) {
        if(choose_mat < 0.8) {
          list[i++] = new Moving_Sphere(
              center, center + Eigen::Vector3f(0, 0.5 * drand48(), 0), 0.0, 1.0, 0.2,
              new Lambertian(new ConstantTexture(Eigen::Vector3f(drand48() * drand48(), drand48() * drand48(),
                                                      drand48() * drand48()))));
          // list[i++] = new Sphere(
          //     center, 0.2,
          //     new Lambertian(Eigen::Vector3f(drand48() * drand48(), drand48() * drand48(),
          //                         drand48() * drand48())));
        } else if (choose_mat < 0.95) {
          list[i++] = new Sphere(center, 0.2, new Metal(Eigen::Vector3f(0.5*(1+drand48()), 0.5*(1+drand48()),0.5*(1+drand48())), 0.5*drand48()));
        } else {
  ...
  list[i++] = new Sphere(Eigen::Vector3f(-4, 1, 0), 1.0, new Lambertian(new ConstantTexture(Eigen::Vector3f(0.4, 0.2, 0.1))));
